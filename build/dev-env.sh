...
minikube image load "quasar-first:${TAG}"

echo "[dev-env] creating quasar-first namespace"
kubectl create namespace quasar-first

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl get namespace quasar-first -n quasar-first"
...
kubectl create -f manifests/persistent-storage-no-pvc-deletion.yaml -n quasar-first

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl rollout status deployment.apps/rfs-redisfailover-persistent-keep -n quasar-first"
until $ROLLOUT_STATUS_CMD || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done
...
STATEFULSET_REPLICAS=$(kubectl get statefulset rfr-redisfailover-persistent-keep -o jsonpath='{.spec.replicas}' -n quasar-first)
ATTEMPTS=0
until [[ ${STATEFULSET_REPLICAS} -eq $(kubectl get statefulset rfr-redisfailover-persistent-keep -o jsonpath='{.status.readyReplicas}' -n quasar-first) ]] || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done

echo "[dev-env] installing quasar-first charts"
helm upgrade --install \
    quasar-first charts/quasar-first \
    --namespace quasar-first \
    --set ingress.host.name="quasar-first.${INGRESS_HOST}"

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl rollout status deployment/quasar-first -n quasar-first"
until $ROLLOUT_STATUS_CMD || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done
...
FastAPI available under: http://quasar-first.${INGRESS_HOST}/
