...
Now you should access CLI script: $ quasar-first --help
Alternatively you can access CLI script via poetry run: $ poetry run quasar-first --help
To deactivate virtualenv simply type: $ deactivate
To activate shell completion:
 - for bash: $ echo 'eval "$(_QUASAR_FIRST_COMPLETE=source_bash quasar-first)' >> ~/.bashrc
 - for zsh: $ echo 'eval "$(_QUASAR_FIRST_COMPLETE=source_zsh quasar-first)' >> ~/.zshrc
 - for fish: $ echo 'eval "$(_QUASAR_FIRST_COMPLETE=source_fish quasar-first)' >> ~/.config/fish/completions/quasar-first.fish
EOF