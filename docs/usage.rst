...
.. code-block:: bash
    ...
    $ quasar-first --help
    Usage: quasar-first [OPTIONS] COMMAND [ARGS]...

      Quasar-first CLI root.
    ...
    Commands:
      serve  quasar-first CLI serve command.
    $ quasar-first serve --help
    Usage: quasar-first serve [OPTIONS]
...
.. code-block:: bash
    ...
    $ make image
    $ podman run -dit --name quasar-first -p 8000:8000 quasar-first:$(cat TAG)
    f41e5fa7ffd512aea8f1aad1c12157bf1e66f961aeb707f51993e9ac343f7a4b
    $ podman ps
    CONTAINER ID  IMAGE                                 COMMAND               CREATED        STATUS            PORTS                   NAMES
    f41e5fa7ffd5  localhost/quasar-first:0.1.0  /usr/bin/fastapi ...  2 seconds ago  Up 3 seconds ago  0.0.0.0:8000->8000/tcp  quasar-first
    $ curl localhost:8000/api/ready
    {"status":"ok"}
...
You can implement your own web routes logic straight away in ``quasar_first.controllers`` submodule. For more information please see `FastAPI documentation <https://fastapi.tiangolo.com/tutorial/>`__.
...
.. code-block:: bash
    ...
    Usage:
      ...
      clean-image      Clean quasar-first image
      install          Install quasar-first with poetry
      metrics          Run quasar-first metrics checks
      unit-test        Run quasar-first unit tests
      integration-test  Run quasar-first integration tests
      docs             Build quasar-first documentation
...
* RedisClient ``quasar_first.app.utils.redis``
* AiohttpClient ``quasar_first.app.utils.aiohttp_client``
...
.. code-block:: python
    ...
    from quasar_first.app.utils import RedisClient
...
.. literalinclude:: ../quasar_first/app/exceptions/http.py
    :language: python
...
All application configuration is available in ``quasar_first.config`` submodule.
...
.. literalinclude:: ../quasar_first/config/gunicorn.py
    :language: python

Routes
~~~~~~

Endpoints are defined in ``quasar_first.app.router`` submodule. Just simply import your controller and include it to FastAPI router:

.. literalinclude:: ../quasar_first/app/router.py
    :language: python
